...
//Modules
import { formatTax, filterTaxes } from './ncar_invoices.controller/formatTaxes'
...
export const getBatchOfInvoices = async (req, res) => {
    ...
    FROM ARLEDG
WHERE invoice BETWEEN '${invoice1}' AND '${invoice1}' AND srccode='NC' GROUP BY INVOICE, ENTITYID, ACCOUNTID
ORDER BY invoice`
...
const extraFields = async (invcnumber) => {
	...
	from arledg  WHERE GOVINVC = '${invcnumber}' and SRCCODE = 'CH'`
...
export const createInvoice = async (bathOfInvoices, crearFactura, req, res) => {
	...
	let dir = `NC AR - notaDeCredito-${timestamp}`
...
export const sendInvoices = async (req, res) => {
	...
	try {
		const response = await fetch(`http://localhost:3000/package/create/NC-AR/${username}`, {
			method: 'POST',
			headers: {
